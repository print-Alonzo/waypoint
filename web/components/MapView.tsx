...
// Loaded client-only via next/dynamic({ ssr: false }) — Leaflet needs `window`.
// The text list remains the accessible + print source of truth; this is a
// supplementary visual that lets testers verify the route makes geographic sense.
...
export default function MapView({ stops, start }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    // Route line through start → every stop, in visit order.
    if (points.length > 1) {
      L.polyline(points, {
        color: lineColor,
        weight: 3,
  ...
  }, [stops, start.lat, start.lng, start.name])
